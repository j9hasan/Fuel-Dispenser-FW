...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
/* How often (ms) to re-run the SIM800L connectivity/data test in the main
 loop. AT+CGATT? / AT+CREG? are cheap, but keep this a few seconds so the
 loop isn't dominated by UART polling. */
#define SIM800L_TEST_INTERVAL_MS  10000U
/* USER CODE END PD */
...
static uint32_t sim800l_last_check = 0U;
/* USER CODE END PV */
