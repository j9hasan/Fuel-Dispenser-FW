
# ZCheng (Zhejiang Genuine) RS-485 Poller
# Feature: 4.10 Accumulative totals (volume + sales)
# Protocol: A5 header, CRC8 checksum, per Genuine Protocol PDF
# Baud: 4800 8N1

import serial

COM_PORT = "COM3"   # Change to your COM port
BAUD = 4800
TIMEOUT = 0.1 
DEVICE_ADDR = 0x01

CRC8_TAB = [
    ,
]

def crc8(data: bytes) -> int:
    c=0
    for b in data: c = CRC8_TAB[c ^ b]
    return c

def build_read(addr:int,origin:int,length:int)->bytes:
    body=bytes([addr,0x03,origin,length])
    return b"\xA5"+body+bytes([crc8(body)])
# bytes([crc8(body)])
def parse_slave(frame: bytes):
    if not frame or frame[0] != 0xA5:
        return None
    addr, func, ln = frame[1], frame[2], frame[3]
    # --- error frame check ---
    if func & 0x80:
        err_code = ln  # here 'length' actually holds the error code
        print(f"[ERROR] func={func:02X}, code={err_code:02X}")
        return None
    data = frame[4:4+ln]
    crc_rx = frame[-1]
    if crc8(bytes([addr, func, ln]) + data) != crc_rx:
        return None
    return addr, func, data


def bcd_to_float(b: bytes, decimals=2):
    digits="".join(f"{x:02X}" for x in b).lstrip("0") or "0"
    if len(digits)<=decimals: digits=digits.zfill(decimals+1)
    return float(f"{int(digits[:-decimals])}.{digits[-decimals:]}")

def read_accum_totals(ser):
    pkt = build_read(DEVICE_ADDR, 0x00, 0x01)
    ser.reset_input_buffer()
    ser.write(pkt)
    # Expect 1 + 3 + 12 + 1 = 17 bytes
    resp = ser.read(17)
    print("RAW reply:", resp.hex())
    parsed = parse_slave(resp)
    print(parsed)
    if not parsed:
        print("Failed to parse accumulative totals (error or CRC mismatch).")
        return None
    d = parsed[2]
    if len(d)!=12:
        print("Invalid data length:", len(d))
        return None
    vol = bcd_to_float(d[0:6])
    sale = bcd_to_float(d[6:12])
    return vol,sale

def main():
    while(True):
        with serial.Serial(COM_PORT,BAUD,timeout=TIMEOUT) as ser:
            result = read_accum_totals(ser)
            print(result)
            if result:
                print(f"[TOTAL] Volume={result[0]:.2f} L  Sale={result[1]:.2f}")
            else:
                print("[TOTAL] Not available.")

if __name__=="__main__":
    main()
